...
public class UserController {
    ...
    @ApiOperation(value = "登录接口", notes = "根据用户名密码登录，登录前会判断该账号是否存在")
    @PostMapping("login.do")
    public ResultVo login(String username,String password){
        return userService.login(username,password);
    }

    @ApiOperation(value = "检查该用户是否存在", notes = "根据用户输入的用户名，判断该用户是否存在")
    @PostMapping("checkUser.do")
    public ResultVo checkUser(String username){
        return userService.checkUser(username);
    }

    @ApiOperation(value = "用户注册",notes = "只需要传入username和password的值")
